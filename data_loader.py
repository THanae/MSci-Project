...
def load_data(df_name: str = 'Lb_data') -> pd.DataFrame:
    """
...
def add_branches(df_name: str = 'Lb_data') -> List[str]:
    """
    ...
    if df_name == 'B_MC':
        mc = ['Lb_MC_MOTHER_ID', 'pKmu_MC_MOTHER_ID', 'proton_MC_MOTHER_ID', 'mu1_MC_MOTHER_ID', 'tauMu_MC_MOTHER_ID',
              ...
              'proton_MC_GD_GD_MOTHER_KEY', 'Kminus_MC_GD_GD_MOTHER_KEY', 'tauMu_MC_GD_GD_MOTHER_KEY',
              'mu1_MC_GD_GD_MOTHER_KEY',
    ...
    for i in range(len(data_frame)):
        for j in range(len(data_frame)):
            if i == j:
                ...
                taux = (1 - tau_x_percentage) * data_frame['tauMu_PX'][i] <= data_frame['mu1_PX'][j] <= (
                        1 + tau_x_percentage) * data_frame['tauMu_PX'][i]
                tau_y_percentage = sigma * data_frame['tauMu_P_COVYY'][i] / data_frame['tauMu_PY'][i]
                tauy = (1 - tau_y_percentage) * data_frame['tauMu_PY'][i] <= data_frame['mu1_PY'][j] <= (
                        1 + tau_y_percentage) * data_frame['tauMu_PY'][i]
                tau_z_percentage = sigma * data_frame['tauMu_P_COVZZ'][i] / data_frame['tauMu_PZ'][i]
                tauz = (1 - tau_z_percentage) * data_frame['tauMu_PZ'][i] <= data_frame['mu1_PZ'][j] <= (
                        1 + tau_z_percentage) * data_frame['tauMu_PZ'][i]
                taux_dup = (1 - tau_x_percentage) * data_frame['tauMu_PX'][i] <= data_frame['tauMu_PX'][j] <= (
                        1 + tau_x_percentage) * data_frame['tauMu_PX'][i]
                tauy_dup = (1 - tau_y_percentage) * data_frame['tauMu_PY'][i] <= data_frame['tauMu_PY'][j] <= (
                        1 + tau_y_percentage) * data_frame['tauMu_PY'][i]
                tauz_dup = (1 - tau_z_percentage) * data_frame['tauMu_PZ'][i] <= data_frame['tauMu_PZ'][j] <= (
                        1 + tau_z_percentage) * data_frame['tauMu_PZ'][i]
                if px and py and pz and kx and ky and kz:
                    print(i, j)
                    combinations.append([i, j])
                if px and py and pz and kx and ky and kz and mux and muy and muz and taux and tauy and tauz:
                    print('mu and tau inverted', i, j)
                if px and py and pz and kx and ky and kz and mux_dup and muy_dup and muz_dup and taux_dup and tauy_dup and tauz_dup:
                    print('clones', i, j)
                if px and py and pz and kx and ky and kz and mux_dup and muy_dup and muz_dup:
                    print('mu dups', i, j)
                if px and py and pz and kx and ky and kz and taux_dup and tauy_dup and tauz_dup:
                    print('tau dups', i, j)
    print(combinations)
    print(len(combinations))
    combinations = np.sort(combinations)
    print(np.unique(combinations, axis=0))
    return np.unique(combinations, axis=0)
